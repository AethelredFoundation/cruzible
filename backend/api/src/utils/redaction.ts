...
/**
 * Error names whose messages are generated by the JS runtime itself
 * (property-access shapes, numeric bounds, parse positions). They cannot
 * embed connection strings or credentials, so they are safe to log.
 */
...
function redactValue(
  ...
  if (value instanceof Error) {
    // Name alone made every operational failure undiagnosable ("TypeError"
    // and nothing else). The policy line: messages of RUNTIME-generated
    // errors (TypeError, RangeError, ...) are produced by the JS engine from
    // code shapes and cannot carry connection strings or credentials — those
    // are included. Library/IO error messages stay hidden (they embed URLs,
    // hosts, and auth material). One stack frame locates the throw site.
    const redactedError: Record<string, RedactedJsonValue> = {
      errorName: value.name,
    };
    if (RUNTIME_ERROR_NAMES.has(value.name)) {
      redactedError.errorMessage = value.message;
    }
    const frame = value.stack?.split("\n")[1]?.trim();
    if (frame) {
      redactedError.errorAt = frame;
    }

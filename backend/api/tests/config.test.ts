...
describe('backend config hardening', () => {
  it('rejects development JWT secrets in production', async () => {
    ...
    ).rejects.toThrow('Refusing to start with development JWT secrets in production');
  });
  ...
  it('rejects wildcard CORS in production', async () => {
    ...
    ).rejects.toThrow('Refusing to start with wildcard CORS origins in production');
  });

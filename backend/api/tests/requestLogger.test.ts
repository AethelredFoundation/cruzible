...
describe("request logger redaction", () => {
  ...
  it("redacts sensitive referer values from standard request logs", async () => {
    const app = express();
    app.use(requestLogger);
    ...
    await withHttpServer(app, async (baseUrl) => {
      const response = await fetch(`${baseUrl}/target`, {
        headers: {
          Referer:
            "https://app.example/callback?refresh_token=refresh-123&address=aeth1user#access_token=token-123",
        },
      });
      ...
      expect(logger.info).toHaveBeenCalledWith(
        "HTTP request completed",
        expect.objectContaining({
          referer: "/callback?refresh_token=[REDACTED]&address=aeth1user",
        }),
      );
    });
  });

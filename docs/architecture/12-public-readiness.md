...
| Kubernetes readiness               | Partial    | Frontend, API gateway, and indexer manifests are checked in with fail-closed config/secret requirements, read-only roots, bounded `/tmp` write surfaces, secret file permissions, and ephemeral-storage budgets; staging validation is still required |
...
- Stage-test `k8s/base/` with real `cruzible-api-config` values, a provisioned `cruzible-api-secrets` Secret, and workload checks that confirm the bounded `/tmp` mounts are sufficient for runtime writes.

...
| Contracts | Canonical EVM liquid-staking vault, stAETHEL receipt token, and wstAETHEL wrapper; the CosmWasm workspace is an earlier non-deployed track                                                                                          |
| Testing   | Frontend Vitest, API Vitest, canonical EVM Foundry tests and committed-artifact integrity checks, plus Cargo tests for the earlier Cosmos-native track                                                                            |
...
| Deployment scaffolding             | Partial    | Compose uses immutable API/indexer images, checked-in nginx, Prometheus, Grafana, and PostgreSQL baselines, plus an operator-managed external `rediss://` endpoint; CI validates digest images, localhost-only internal ports, external Redis, and externalized secret files; operator secrets, TLS material, immutable image digests, and staging validation are still required                                      |
...
- Capture a canonical EVM staging deployment manifest with source commit, bytecode checksums, transaction hashes, contract addresses, and role owners. CosmWasm release evidence applies only if the earlier Cosmos-native track is deliberately revived.
- Activate and verify the chain staking/distribution precompiles on the exact public-testnet binary before exercising delegation, reward claims, undelegation, or slashing reconciliation.
...
- Build frontend images with `NEXT_PUBLIC_API_URL`, `NEXT_PUBLIC_CHAIN_ENV`, and the selected network's explicit RPC build input; frontend builds fail closed when the API/RPC is missing or unsafe, and mainnet remains unavailable until an explicit confirmed chain ID, RPC, and explorer are supplied.

...
| `NEXT_PUBLIC_API_URL`                        | Required at build time                            | `http://localhost:4001/v1`        | Base URL for frontend API requests; Next.js compiles this into browser bundles, so Kubernetes runtime env alone cannot change it; production mainnet/testnet builds must use the exact approved API origin for the selected chain |
| `NEXT_PUBLIC_AETHELRED_TESTNET_RPC_URL`      | Required for testnet builds                       | operator supplied                 | EVM JSON-RPC compiled into the testnet bundle; no undeployed DNS fallback exists; public variables must not contain credentials                                                                                                   |
| `NEXT_PUBLIC_AETHELRED_GENESIS_HASH`         | Required for network builds                       | operator supplied                 | Non-zero canonical EVM genesis anchor hash (block 1) compiled into the browser bundle and verified before wallet writes; distinguishes testnet and devnet even though both currently use chain ID 7332                            |
| `NEXT_PUBLIC_AETHELRED_DEVNET_RPC_URL`       | Optional for devnet                               | `http://127.0.0.1:8545`           | Local EVM JSON-RPC override                                                                                                                                                                                                       |
| `NEXT_PUBLIC_AETHELRED_MAINNET_CHAIN_ID`     | Required for mainnet builds                       | none                              | No repository mainnet placeholder is accepted; must be the formally confirmed EIP-155 chain ID and must differ from testnet 7332                                                                                                  |
| `NEXT_PUBLIC_AETHELRED_MAINNET_RPC_URL`      | Required for mainnet builds                       | none                              | Formally confirmed mainnet EVM JSON-RPC endpoint                                                                                                                                                                                  |
| `NEXT_PUBLIC_AETHELRED_MAINNET_EXPLORER_URL` | Required for mainnet builds                       | none                              | Formally confirmed mainnet explorer origin                                                                                                                                                                                        |
| `NEXT_PUBLIC_APP_VERSION`                    | No                                                | `local-dev`                       | Displayed in UI and sent in request headers                                                                                                                                                                                       |
| `NEXT_PUBLIC_ZEROID_APP_URL`                 | Optional                                          | blank                             | ZeroID onboarding origin used by the identity-gated vault; production URLs are validated and compiled at build time                                                                                                               |
| `NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID`       | Required for mainnet; optional for testnet/devnet | blank                             | Enables WalletConnect only when set; any supplied value must be a registered 32-character hex Reown project ID. Leave blank on testnet when testing with Aethelred Wallet or MetaMask                                             |
...
| `NEXT_PUBLIC_AETHEL_TOKEN_ADDRESS`           | Required only for mainnet                         | blank                             | Optional bridged-token address; AETHEL is native on the current testnet and this remains blank there                                                                                                                              |
| `NEXT_PUBLIC_GOVERNANCE_ADDRESS`             | Optional                                          | blank                             | Governance remains preview-oriented in this snapshot                                                                                                                                                                              |
| `NEXT_PUBLIC_STABLECOIN_BRIDGE_ADDRESS`      | Required only for mainnet                         | blank                             | Optional on the current testnet; mainnet release validation requires the full confirmed periphery                                                                                                                                 |
| `NEXT_PUBLIC_USDC_TOKEN_ADDRESS`             | Required only for mainnet                         | blank                             | Optional on the current testnet                                                                                                                                                                                                   |
| `NEXT_PUBLIC_USDT_TOKEN_ADDRESS`             | Required only for mainnet                         | blank                             | Optional on the current testnet                                                                                                                                                                                                   |
...
Testnet frontend images must be built with explicit chain and contract config.
WalletConnect is optional: omit its build argument to use only Aethelred Wallet,
MetaMask, and other injected wallets. If WalletConnect testing is required, set
`NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID` to a project ID registered to Aethelred;
do not copy an example or another application's ID.

```bash
docker build \
  --build-arg NEXT_PUBLIC_API_URL=https://api.testnet.aethelred.org \
  --build-arg NEXT_PUBLIC_CHAIN_ENV=testnet \
  --build-arg NEXT_PUBLIC_AETHELRED_TESTNET_RPC_URL=https://<operator-rpc> \
  --build-arg NEXT_PUBLIC_AETHELRED_GENESIS_HASH=0xf4b43647f4d3255a7e9321ea4b32057101ed143623390bc30d59e69a91ceafa7 \
  --build-arg NEXT_PUBLIC_CRUZIBLE_ADDRESS=0x... \
  --build-arg NEXT_PUBLIC_STAETHEL_ADDRESS=0x... \
  -t cruzible-frontend:staging .
```

Mainnet has no repository chain-ID, RPC, or explorer defaults. Do not build a
mainnet image until those values are formally confirmed. At that point the
release requires all three explicit network inputs plus the complete contract
set:
...
docker build \
  ...
  --build-arg NEXT_PUBLIC_AETHELRED_MAINNET_CHAIN_ID=<confirmed-id> \
  --build-arg NEXT_PUBLIC_AETHELRED_MAINNET_RPC_URL=https://<confirmed-rpc> \
  --build-arg NEXT_PUBLIC_AETHELRED_MAINNET_EXPLORER_URL=https://<confirmed-explorer> \
  --build-arg NEXT_PUBLIC_AETHELRED_GENESIS_HASH=0x<confirmed-64-hex-block-1-hash> \
  --build-arg NEXT_PUBLIC_CRUZIBLE_ADDRESS=0x... \
  --build-arg NEXT_PUBLIC_STAETHEL_ADDRESS=0x... \
...
Vercel previews retain deterministic non-network placeholders for UI review but
must receive an explicit `NEXT_PUBLIC_AETHELRED_TESTNET_RPC_URL`; the build no
longer compiles an undeployed fallback. Vercel production deployments must
configure the full selected-network build contract.

The browser-facing `NEXT_PUBLIC_*` values above are immutable once the image is
built. Two server-side response-policy inputs remain intentionally configurable
at runtime because Next.js middleware reads them when it emits CSP and HSTS
headers:

| Variable                        | Required | Safe default | Notes                                                                                                                           |
| ------------------------------- | -------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------- |
| `CRUZIBLE_EXTRA_API_ORIGINS`    | No       | blank        | Comma-separated additional API origins admitted by `connect-src`; use only the exact origins approved during release validation |
| `CRUZIBLE_ALLOW_PLAINTEXT_HTTP` | No       | `false`      | Pre-TLS testnet escape hatch; production and all TLS deployments must keep this `false`                                         |

For a pre-TLS testnet, inject the same narrowly scoped values when creating the
release candidate and at container runtime. The candidate inventory attests
both values in its canonical build-configuration fingerprint, and promotion
fails unless the destination ConfigMap matches them exactly. A runtime-policy
change therefore requires a new candidate; an existing image must not be
reconfigured to an unattested policy:

```bash
docker run --rm -p 3000:3000 \
  -e CRUZIBLE_EXTRA_API_ORIGINS=http://93.127.132.52:4001 \
  -e CRUZIBLE_ALLOW_PLAINTEXT_HTTP=true \
  cruzible-frontend@sha256:<approved-digest>
```

Remove both overrides as soon as TLS is available. Never use a wildcard origin.
...
| `PORT`                                        | No                                                           | `4001`                                                                   | HTTP and Socket.IO listen port                                                                                                                                                                                                    |
...
| `INDEXER_RPC_URL`                             | Required for production indexer and API vault reconciliation | `http://127.0.0.1:8545`                                                  | JSON-RPC endpoint used by the indexer and the API's independent same-block vault reconciliation; must use `http://` or `https://`                                                                                                 |
...
| `INDEXER_EXPECTED_GENESIS_HASH`               | Required for production API/indexer                          | blank                                                                    | Immutable block-1 network anchor; verified over HTTP and WebSocket and included in wallet-login domain separation because testnet and devnet share chain ID 7332                                                                  |
...
| `STABLECOIN_BRIDGE_ADDRESS`                   | Required for mainnet/all-periphery indexer profile           | blank                                                                    | Optional on the public-testnet profile until the bridge is deployed; otherwise must be a non-zero EVM address                                                                                                                     |
...
| `RECONCILIATION_RATE_WARN_PCT`                | No                                                           | `0.01`                                                                   | Warning threshold for indexed-versus-independent live exchange-rate mismatch at the same indexed block; a rate above 1.0 from rewards is healthy when the two sources agree                                                       |
| `RECONCILIATION_RATE_CRIT_PCT`                | No                                                           | `0.05`                                                                   | Critical same-block exchange-rate mismatch threshold; must be greater than `RECONCILIATION_RATE_WARN_PCT`                                                                                                                         |
| `RECONCILIATION_TVL_DRIFT_PCT`                | No                                                           | `0.02`                                                                   | Indexed-versus-live vault `totalPooledAethel` mismatch threshold; network-wide validator stake is not used as the vault baseline                                                                                                  |
...
| `INDEXER_RPC_URL`                     | api-gateway / indexer          | JSON-RPC endpoint used by indexing and independent same-block vault reconciliation                            |
...
| `INDEXER_EXPECTED_GENESIS_HASH`       | api-gateway, indexer           | Required immutable network-anchor guard and wallet-login domain separator                                     |
| `INDEXER_HEARTBEAT_FILE`              | indexer                        | Process-watchdog file; production manifests pin it to `/tmp/cruzible-indexer-heartbeat.json`                  |
| `INDEXER_HEARTBEAT_MAX_AGE_MS`        | indexer                        | Maximum heartbeat age used by container/Kubernetes probes; production manifests pin `45000`                   |
| `CRUZIBLE_VAULT_ADDRESS`              | api-gateway / indexer          | Required non-zero vault contract address                                                                      |
...
| `CRUZIBLE_MIGRATION_IMAGE_DIGEST`     | database migration             | Immutable first-party migration image digest from the same completed release inventory                        |
| `CRUZIBLE_MIGRATION_QUIESCED`         | database migration             | Set to `true` only after every old indexer process has stopped and drained                                    |
| `CRUZIBLE_LEGACY_SCHEDULERS_QUIESCED` | database migration             | Set to `true` only after every pre-namespace API reconciliation scheduler has stopped and drained             |
...
- `backend/infra/docker-compose.yml` intentionally does not bundle Redis. The `REDIS_URL_FILE` secret must identify an externally managed TLS endpoint (`rediss://`), matching the production runtime guard.
- `backend/infra/docker-compose.yml` binds internal API, database, RPC, and observability ports to loopback by default; only nginx is public-facing in the scaffold.

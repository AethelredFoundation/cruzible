...
| `NEXT_PUBLIC_STAETHEL_ADDRESS`          | Required for mainnet        | blank                             | Liquid staking receipt token address; mainnet builds require a non-zero EVM address                                              |
| `NEXT_PUBLIC_AETHEL_TOKEN_ADDRESS`      | Required for mainnet        | blank                             | Underlying token address; mainnet builds require a non-zero EVM address                                                          |
| `NEXT_PUBLIC_GOVERNANCE_ADDRESS`        | Optional                    | blank                             | Governance remains preview-oriented in this snapshot                                                                             |
| `NEXT_PUBLIC_STABLECOIN_BRIDGE_ADDRESS` | Required for mainnet        | blank                             | Stablecoin bridge contract address; mainnet builds require a non-zero EVM address                                                |
| `NEXT_PUBLIC_USDC_TOKEN_ADDRESS`        | Required for mainnet        | blank                             | Stablecoin token address; mainnet builds require a non-zero EVM address                                                          |
| `NEXT_PUBLIC_USDT_TOKEN_ADDRESS`        | Required for mainnet        | blank                             | Stablecoin token address; mainnet builds require a non-zero EVM address                                                          |
| `NEXT_PUBLIC_ENABLE_DEVTOOLS`           | Optional                    | `false`                           | Local/test only; must be explicitly `true`; production hard-disables `/devtools` even if this public flag is set                 |
...
explicit `NEXT_PUBLIC_API_URL` and `NEXT_PUBLIC_CHAIN_ENV` values.
...
| `JWT_SECRET`                        | Yes                                    | replace with secret                   | Development defaults and values shorter than 32 characters are rejected in production; must be distinct from `JWT_REFRESH_SECRET` and `OPERATIONAL_ENDPOINTS_TOKEN` |
| `JWT_REFRESH_SECRET`                | Yes                                    | replace with secret                   | Development defaults and values shorter than 32 characters are rejected in production; must be distinct from `JWT_SECRET` and `OPERATIONAL_ENDPOINTS_TOKEN` |
| `JWT_EXPIRES_IN`                    | No                                     | `15m`                                 | Must match the `^\d+[mhd]$` pattern; production rejects values longer than `15m`                                                                         |
...
| `OPERATIONAL_ENDPOINTS_TOKEN`       | Required in production                 | blank                                 | Bearer or `X-Operational-Token` credential for full `/health`, `/metrics`, `/docs`, and operational WebSocket handshakes; must be at least 32 characters and distinct from JWT signing secrets |
...
- Production cookie-authenticated refresh/logout requests require a trusted `Origin` or `Referer` that matches `CORS_ORIGINS`.

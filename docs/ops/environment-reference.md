...
| `NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID`  | Required for mainnet        | blank                             | Needed for WalletConnect flows; mainnet builds require a 32-character hex project ID                                                                                                                                              |
...
| `OPERATIONAL_ENDPOINTS_TOKEN`       | Required in production                 | blank                                                                    | Bearer or `X-Operational-Token` credential for full `/health`, `/metrics`, `/docs`, and operational WebSocket handshakes; must be at least 32 characters, contain no control characters, and be distinct from JWT signing secrets |
